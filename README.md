...
- 全链路灰度调用链。包括Header方式、Opentracing方式、日志方式等单个或者组合式的全链路灰度调用链。Opentracing方式不支持Edgware版（Spring Boot 1.x），不支持Finchley版（Spring Boot 2.0.x）的Spring Cloud Gateway，除此之外的场景都支持
- 全链路Header传递
